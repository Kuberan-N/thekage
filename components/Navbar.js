    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm border-b border-gray-100">
        <div className="flex items-center justify-between px-4 h-14 max-w-screen-xl mx-auto">
          ...
          {/* Mobile left: nothing (logo is centered) */}
          ...
          <div className="flex items-center gap-1">
            ...
            {/* Desktop Hamburger (right) — hidden on mobile */}
            <button
              className="md:hidden flex items-center justify-center w-9 h-9 hover:opacity-60 transition-opacity"
              onClick={() => setMenuOpen(!menuOpen)}
              aria-label="Menu"
            >
              <svg width="20" height="14" viewBox="0 0 20 14" fill="none">
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-[100] bg-white flex flex-col">
          <div className="flex items-center justify-between px-4 h-14 border-b border-gray-100">
            <span className="font-black text-xl tracking-tighter uppercase">BLUORNG</span>
            <button
          ...
          <nav className="flex flex-col px-6 pt-8 gap-6">
            {["Shop", "Latest Drop", "Bestsellers", "Hoodies", "T-Shirts", "Caps", "About"].map((item) => (
              <Link
                key={item}
                href="/"
                onClick={() => setMenuOpen(false)}
          ...
          <div className="mt-auto px-6 pb-10 flex gap-6">
            {["Instagram", "WhatsApp", "YouTube"].map((s) => (
              <a key={s} href="#" className="text-xs text-gray-400 hover:text-black transition-colors uppercase tracking-widest">
                {s}
              </a>
            ))}
          </div>
